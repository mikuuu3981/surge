...
6. 保持现有 Surge 节点信息、订阅、systemd/OpenRC 和发行版兼容能力。
...
最低允许版本为 v1.19.28。安装和核心升级只选择不低于该版本的稳定版。
...
- 从 `MetaCubeX/mihomo` GitHub Release 获取最新稳定版。
- 根据主机架构选择 Linux 资产；AMD64 优先兼容构建，ARM64 使用对应原生构建。
- 使用 GitHub Release 资产摘要验证下载；没有可验证摘要时拒绝安装。
- 版本查询使用 Mihomo 自身版本命令。
- 升级前备份当前二进制；安装后先校验现有配置，再重启并检查所有监听端口。
- 升级失败时恢复旧二进制并恢复服务。
...
7. 所有生成配置通过 Mihomo 校验，脚本通过 Bash 语法与帮助路径检查。
